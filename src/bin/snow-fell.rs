...
impl Mapping {
	fn new(subnet: Ipv6Net) -> Result<Self> {
		...
		if min_prefix_len > subnet.prefix_len() as u32 {
			return Err(eyre!("Need at most /{min_prefix_len} subnet for a system with {} sctp_assoc_t bits, found /{}", sctp_assoc_t::BITS, subnet.prefix_len()));
		}
		Ok(Self { subnet })
	}
	...
	fn to_index(&self, addr: Ipv6Addr) -> Option<sctp_assoc_t> {
		if !self.subnet.contains(&addr) { return None };
		...
		for e in events.into_iter() {
			match e.token() {
				...
				TUN => loop {
					...
					let Some(assoc_id) = mapping.to_index(ip.dst.into()) else { continue };
					// Tunnel the packet
					let mut control = [0u8; unsafe { CMSG_SPACE(size_of::<sctp_sndinfo>() as socklen_t) + CMSG_SPACE(size_of::<sctp_prinfo>() as socklen_t) } as usize];
					...
					let res = unsafe {
						...
						write_unaligned(cmsg, cmsghdr { 
						 ...
						 write_unaligned(CMSG_DATA(cmsg).cast(), sctp_prinfo {
				...
				SCTP => loop {
					...
					if msg.msg_flags & MSG_NOTIFICATION != 0 {
						...
						if header.sn_type == sctp_sn_type::SCTP_ASSOC_CHANGE as u16 {
							...
							let change: sctp_assoc_change = unsafe { read_unaligned(data.as_ptr().cast()) };
							let info = &data[size_of_val(&change)..];
							trace!(?change, ?info, "assoc change");

							if change.sac_state != sctp_sac_state::SCTP_COMM_UP as u16 { continue }

							// Open a datachannel
							let Some(addr) = mapping.from_index(change.sac_assoc_id) else { continue };
					...
					// Handle SCTP messages
						...
						let Some(addr6) = unsafe {
							SockAddr::new(transmute(addr), msg.msg_namelen)
						}.as_socket_ipv6() else { continue };
						...
						match (ppid, stream) {
							...
							(51, _) => {
								...
								trace!(?assoc_id, ?addr6, ?udp_port, ?stream, ?data, "String message");
							}
							// VPN traffic
							(53, 1) => {
								let Ok((ip6, _)) = Ip6Header::ref_from_prefix(data) else { continue };
								if ip6.flags.version() != 6 { continue }
								if ip6.len() != data.len() { continue }
								let Some(expected) = mapping.from_index(assoc_id) else { continue };
								if ip6.src != expected.octets() { continue };
